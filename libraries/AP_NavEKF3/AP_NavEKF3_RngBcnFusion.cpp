        ...
        if (PV_AidingMode == AID_ABSOLUTE) {
            if ((frontend->sources.getPosXYSource() == AP_NavEKF_Source::SourceXY::BEACON) && rngBcnAlignmentCompleted) {
                if (!bcnOriginEstInit) {
                    ...
                    bcnPosOffsetNED.x = -beaconVehiclePosNED.x;
                    bcnPosOffsetNED.y = -beaconVehiclePosNED.y;
                    receiverPos.x = stateStruct.position.x;
                    receiverPos.y = stateStruct.position.y;

                    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Align     x:%.3f  y:%.3f",bcnPosOffsetNED.x,bcnPosOffsetNED.y);
                    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "state     x:%.3f  y:%.3f ",stateStruct.position.x,stateStruct.position.y);
                    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "id %d rec.x:%.3f  y:%.3f ",core_index,receiverPos.x,receiverPos.y);
                }
