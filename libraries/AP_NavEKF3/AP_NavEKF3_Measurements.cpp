...
void NavEKF3_core::readRangeFinder(void) {
  ...
  // don't allow range to go below 5cm because this can cause problems with
  ...
  if ((imuSampleTime_ms - lastRngMeasTime_ms) >
      frontend->sensorIntervalMin_ms) {
    ...
    for (uint8_t sensorIndex = 0; sensorIndex < ARRAY_SIZE(rngMeasIndex);
         sensorIndex++) {
      ...
      if ((sensor->orientation() == ROTATION_PITCH_270) &&
          (sensor->status() == AP_DAL_RangeFinder::Status::Good)) {
        rngMeasIndex[sensorIndex]++;
        if (rngMeasIndex[sensorIndex] > 2) {
          rngMeasIndex[sensorIndex] = 0;
        }
        storedRngMeasTime_ms[sensorIndex][rngMeasIndex[sensorIndex]] =
            imuSampleTime_ms - 25;
        storedRngMeas[sensorIndex][rngMeasIndex[sensorIndex]] =
            sensor->distance();
      } else {
        continue;
      }
      ...
      for (uint8_t index = 0; index <= 2; index++) {
        sampleFresh[sensorIndex][index] =
            (imuSampleTime_ms - storedRngMeasTime_ms[sensorIndex][index]) < 500;
      }

      // find the median value if we have three fresh samples
      if (sampleFresh[sensorIndex][0] && sampleFresh[sensorIndex][1] &&
          sampleFresh[sensorIndex][2]) {
        ...
        if (storedRngMeas[sensorIndex][2] >
            storedRngMeas[sensorIndex][maxIndex]) {
          midIndex = maxIndex;
        } else if (storedRngMeas[sensorIndex][2] <
                   storedRngMeas[sensorIndex][minIndex]) {
          midIndex = minIndex;
        } else {
          midIndex = 2;
        }
        ...
        if (storedRngMeasTime_ms[sensorIndex][midIndex] >
            rangeDataNew.time_ms) {
        ...
        // write data to buffer with time stamp to be fused when the fusion time
        // horizon catches up with it
...
void NavEKF3_core::writeBodyFrameOdom(float quality, const Vector3f &delPos,
                                      const Vector3f &delAng, float delTime,
                                      uint32_t timeStamp_ms, uint16_t delay_ms,
                                      const Vector3f &posOffset) {
#if EK3_FEATURE_BODY_ODOM
  // protect against NaN
  if (isnan(quality) || delPos.is_nan() || delAng.is_nan() || isnan(delTime) ||
      posOffset.is_nan()) {
    return;
  }
  ...
  if (((timeStamp_ms - bodyOdmMeasTime_ms) < frontend->sensorIntervalMin_ms) ||
      (delTime < dtEkfAvg) || !statesInitialised) {
    return;
  }
  ...
  bodyOdmDataNew.velErr =
      frontend->_visOdmVelErrMin +
...
void NavEKF3_core::writeWheelOdom(float delAng, float delTime,
                                  uint32_t timeStamp_ms,
                                  const Vector3f &posOffset, float radius) {
#if EK3_FEATURE_BODY_ODOM
  // This is a simple hack to get wheel encoder data into the EKF and verify the
  // interface sign conventions and units It uses the exisiting body frame
  // velocity fusion.
  ...
  // because we are currently converting to an equivalent velocity measurement
  // before fusing the measurement time is moved back to the middle of the
  // sampling period
...
void NavEKF3_core::writeOptFlowMeas(const uint8_t rawFlowQuality,
                                    const Vector2f &rawFlowRates,
                                    const Vector2f &rawGyroRates,
  ...
  // The raw measurements need to be optical flow rates in radians/second
  // averaged across the time since the last update The PX4Flow sensor outputs
  // flow rates with the following axis and sign conventions: A positive X rate
  // is produced by a positive sensor rotation about the X axis A positive Y
  // rate is produced by a positive sensor rotation about the Y axis This filter
  // uses a different definition of optical flow rates to the sensor with a
  // positive optical flow rate produced by a negative rotation about that axis.
  // For example a positive rotation of the flight vehicle about its X (roll)
  // axis would produce a negative X flow rate
  flowMeaTime_ms = imuSampleTime_ms;
  // calculate bias errors on flow sensor gyro rates, but protect against spikes
  // in data reset the accumulated body delta angle and time don't do the
  // calculation if not enough time lapsed for a reliable body rate measurement
  if (delTimeOF > 0.01f) {
    flowGyroBias.x =
        0.99f * flowGyroBias.x +
        0.01f * constrain_ftype((rawGyroRates.x - delAngBodyOF.x / delTimeOF),
                                -0.1f, 0.1f);
    flowGyroBias.y =
        0.99f * flowGyroBias.y +
        0.01f * constrain_ftype((rawGyroRates.y - delAngBodyOF.y / delTimeOF),
                                -0.1f, 0.1f);
    delAngBodyOF.zero();
    delTimeOF = 0.0f;
  }
  // by definition if this function is called, then flow measurements have been
  // provided so we need to run the optical flow takeoff detection
  detectOptFlowTakeoff();

  // don't use data with a low quality indicator or extreme rates (helps catch
  // corrupt sensor data)
  if ((rawFlowQuality > 0) && rawFlowRates.length() < 4.2f &&
      rawGyroRates.length() < 4.2f) {
    ...
    // the sensor interface doesn't provide a z axis rate so use the rate from
    // the nav sensor instead
    if (delTimeOF > 0.001f) {
      // first preference is to use the rate averaged over the same sampling
      // period as the flow sensor
    ...
    // note correction for different axis and sign conventions used by the
    // px4flow sensor
    ofDataNew.flowRadXY =
        -rawFlowRates.toftype(); // raw (non motion compensated) optical flow
                                 // angular rate about the X axis (rad/sec)
    ...
    // record time last observation was received so we can detect loss of data
    // elsewhere
    flowValidMeaTime_ms = imuSampleTime_ms;
    // estimate sample time of the measurement
    ofDataNew.time_ms = imuSampleTime_ms - frontend->_flowDelay_ms -
                        frontend->flowTimeDeltaAvg_ms / 2;
...
void NavEKF3_core::tryChangeCompass(uint8_t mag_index) {
  ...
  // if the magnetometer is allowed to be used for yaw and has a different
  // index, we start using it
  if (compass.healthy(mag_index) && compass.use_for_yaw(mag_index) &&
      mag_index != magSelectIndex) {
    magSelectIndex = mag_index;
    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u switching to compass %u",
    ...
    storedMag.reset();
    // clear the data waiting flag so that we do not use any data pending from
    // the previous sensor
    magDataToFuse = false;
...
void NavEKF3_core::readMagData() {
  ...
  // If we are a vehicle with a sideslip constraint to aid yaw estimation and we
  // have timed out on our last avialable magnetometer, then declare the
  // magnetometers as failed for this flight
  const uint8_t maxCount = compass.get_count();
  if (allMagSensorsFailed || (magTimeout && assume_zero_sideslip() &&
                              magSelectIndex >= maxCount - 1 && inFlight)) {
    allMagSensorsFailed = true;
    return;
  }
  ...
  } else if (wasLearningCompass_ms != 0 &&
             imuSampleTime_ms - wasLearningCompass_ms > 1000) {
    // allow time for old data to clear the buffer before signalling other code
    // that compass data can be used
  ...
  // If the magnetometer has timed out (been rejected for too long), we find
  // another magnetometer to use if available Don't do this if we are on the
  // ground because there can be magnetic interference and we need to know if
  // there is a problem before taking off. Don't do this within the first 30
  // seconds from startup because the yaw error could be due to large yaw gyro
  // bias affsets if the timeout is due to a sensor failure, then declare a
  // timeout regardless of onground status
  if (maxCount > 1) {
    bool fusionTimeout = magTimeout && !onGround &&
  ...
  // limit compass update rate to prevent high processor loading because
  // magnetometer fusion is an expensive step and we could overflow the FIFO
  // buffer
  if (use_compass() && compass.healthy(magSelectIndex) &&
      ((compass.last_update_usec(magSelectIndex) - lastMagUpdate_us) >
       1000 * frontend->sensorIntervalMin_ms)) {

    // detect changes to magnetometer offset parameters and reset states
    Vector3F nowMagOffsets = compass.get_offsets(magSelectIndex).toftype();
    bool changeDetected =
        lastMagOffsetsValid && (nowMagOffsets != lastMagOffsets);
    if (changeDetected) {
      // zero the learned magnetometer bias states
      stateStruct.body_magfield.zero();
      // clear the measurement buffer
 ...
 *  Downsampling is done using a method that does not introduce coning or
 * sculling errors.
 */
void NavEKF3_core::readIMUData(bool startPredictEnabled) {
  const auto &ins = dal.ins();

  // calculate an averaged IMU update rate using a spike and lowpass filter
  // combination
  dtIMUavg = 0.02f * constrain_ftype(ins.get_loop_delta_t(), 0.5f * dtIMUavg,
                                     2.0f * dtIMUavg) +
             0.98f * dtIMUavg;

  // the imu sample time is used as a common time reference throughout the
  // filter
  ...
  // Accumulate the measurement time interval for the delta velocity and angle
  // data
  ...
  // Accumulation using quaternions prevents introduction of coning errors due
  // to downsampling
  imuQuatDownSampleNew.rotate(imuDataNew.delAng);
  imuQuatDownSampleNew.normalize();

  // Rotate the latest delta velocity into body frame at the start of
  // accumulation
  ...
  /*
   * If the target EKF time step has been accumulated, and the frontend has
   * allowed start of a new predict cycle, then store the accumulated IMU data
   * to be used by the state prediction, ignoring the frontend permission if
   * more than twice the target time has lapsed. Adjust the target EKF step time
   * threshold to allow for timing jitter in the IMU data.
   */
  if ((imuDataDownSampledNew.delAngDT >= (EKF_TARGET_DT - (dtIMUavg * 0.5f)) &&
       startPredictEnabled) ||
    ...
    // calculate the achieved average time step rate for the EKF using a
    // combination spike and LPF
    ftype dtNow = constrain_ftype(0.5f * (imuDataDownSampledNew.delAngDT +
                                          imuDataDownSampledNew.delVelDT),
                                  0.5f * dtEkfAvg, 2.0f * dtEkfAvg);
    dtEkfAvg = 0.98f * dtEkfAvg + 0.02f * dtNow;

    // do an addtional down sampling for data used to sample XY body frame drag
    // specific forces
    ...
    imuQuatDownSampleNew[3] = imuQuatDownSampleNew[2] =
        imuQuatDownSampleNew[1] = 0.0f;

    // reset the counter used to let the frontend know how many frames have
    // elapsed since we started a new update cycle
    ...
    correctDeltaAngle(delAngCorrected, imuDataDelayed.delAngDT,
                      imuDataDelayed.gyro_index);
    correctDeltaVelocity(delVelCorrected, imuDataDelayed.delVelDT,
                         imuDataDelayed.accel_index);

  } else {
    // we don't have new IMU data in the buffer so don't run filter updates on
    // this time step
    runUpdates = false;
  }
}
...
bool NavEKF3_core::readDeltaVelocity(uint8_t ins_index, Vector3F &dVel,
                                     ftype &dVel_dt) {
  const auto &ins = dal.ins();
...
void NavEKF3_core::readGpsData() {
  ...
  if (gps.last_message_time_ms(selected_gps) - lastTimeGpsReceived_ms <=
      frontend->sensorIntervalMin_ms) {
    return;
  }
  ...
  // estimate when the GPS fix was valid, allowing for GPS processing and other
  // delays ideally we should be using a timing signal from the GPS receiver to
  // set this time Use the driver specified delay
  float gps_delay_sec = 0;
  gps.get_lag(selected_gps, gps_delay_sec);
  gpsDataNew.time_ms =
      lastTimeGpsReceived_ms - (uint32_t)(gps_delay_sec * 1000.0f);

  // Correct for the average intersampling delay due to the filter updaterate
  gpsDataNew.time_ms -= localFilterTimeStep_ms / 2;

  // Prevent the time stamp falling outside the oldest and newest IMU data in
  // the buffer
  gpsDataNew.time_ms = MIN(MAX(gpsDataNew.time_ms, imuDataDelayed.time_ms),
  ...
  // Use the speed and position accuracy from the GPS if available, otherwise
  // set it to zero. Apply a decaying envelope filter with a 5 second time
  // constant to the raw accuracy data
  ftype alpha = constrain_ftype(
      0.0002f * (lastTimeGpsReceived_ms - secondLastGpsTime_ms), 0.0f, 1.0f);
  ...
  // check if we have enough GPS satellites and increase the gps noise scaler if
  // we don't
  if (gps.num_sats(selected_gps) >= 6 && (PV_AidingMode == AID_ABSOLUTE)) {
    gpsNoiseScaler = 1.0f;
  } else if (gps.num_sats(selected_gps) == 5 &&
  ...
  // Check if GPS can output vertical velocity, vertical velocity use is
  // permitted and set GPS fusion mode accordingly
  if (gpsDataNew.have_vz && frontend->sources.useVelZSource(
                                AP_NavEKF_Source::SourceZ::GPS, core_index)) {
    useGpsVertVel = true;
  } else {
    useGpsVertVel = false;
  }
  ...
  if (frontend->option_is_enabled(NavEKF3::Option::JammingExpected) &&
      (lastTimeGpsReceived_ms - secondLastGpsTime_ms) >
          frontend->gpsNoFixTimeout_ms) {
    const bool doingBodyVelNav =
        (imuSampleTime_ms - prevBodyVelFuseTime_ms < 1000);
    const bool doingFlowNav = (imuSampleTime_ms - prevFlowFuseTime_ms < 1000);
    ;
    const bool canDoWindRelNav = assume_zero_sideslip();
    const bool canDeadReckon = ((doingFlowNav && gndOffsetValid) ||
                                canDoWindRelNav || doingBodyVelNav);
    if (canDeadReckon) {
      // If we can do dead reckoning with a data source other than GPS there is
      // time to wait for GPS alignment checks to pass before using GPS inside
      // the EKF. this gets set back to false in calcGpsGoodToAlign() when GPS
      // checks pass
  ...
  // Set the EKF origin and magnetic field declination if not previously set and
  // GPS checks have passed
  if (gpsGoodToAlign && !validOrigin) {
    Location gpsloc_fieldelevation = gpsloc;
    // if flying, correct for height change from takeoff so that the origin is
    // at field elevation
    if (inFlight) {
    ...
    ekfGpsRefHgt =
        (double)0.01 * (double)gpsloc.alt + (double)outputDataNew.position.z;
  ...
  // convert GPS measurements to local NED and save to buffer to be fused later
  // if we have a valid origin and are not waiting for GPs checks to pass
  if (validOrigin && !waitingForGpsChecks) {
    gpsDataNew.lat = gpsloc.lat;
    gpsDataNew.lng = gpsloc.lng;
    if ((frontend->_originHgtMode & (1 << 2)) == 0) {
      // the height adjustment to match GPS is being achieved by adjusting the
      // origin height
      gpsDataNew.hgt =
          (ftype)((double)0.01 * (double)gpsloc.alt - ekfGpsRefHgt);
    } else {
      // the height adjustment to match GPS is being achieved by adjusting the
      // measurements
...
void NavEKF3_core::readGpsYawData() {
  ...
  if (gps.status(selected_gps) >= AP_DAL_GPS::GPS_OK_FIX_3D &&
      dal.gps().gps_yaw_deg(selected_gps, yaw_deg, yaw_accuracy_deg,
                            yaw_time_ms) &&
    ...
    writeEulerYawAngle(radians(yaw_deg), radians(yaw_accuracy_deg), yaw_time_ms,
                       2);
  }
}
...
// check for new pressure altitude measurement data and update stored
// measurement if available
void NavEKF3_core::readBaroData() {
  // check to see if baro measurement has changed so we know if a new
  // measurement has arrived limit update rate to avoid overflowing the FIFO
  // buffer
  const auto &baro = dal.baro();
  if (baro.get_last_update(selected_baro) - lastBaroReceived_ms >
      frontend->sensorIntervalMin_ms) {
...
// calculate filtered offset between baro height measurement and EKF height
// estimate offset should be subtracted from baro measurement to match filter
// estimate offset is used to enable reversion to baro from alternate height
// data source
void NavEKF3_core::calcFiltBaroOffset() {
  // Apply a first order LPF with spike protection
  baroHgtOffset +=
      0.1f * constrain_ftype(baroDataDelayed.hgt + stateStruct.position.z -
                                 baroHgtOffset,
                             -5.0f, 5.0f);
}

// correct the height of the EKF origin to be consistent with GPS Data using a
// Bayes filter.
void NavEKF3_core::correctEkfOriginHeight() {
  ...
  ftype deltaTime = constrain_ftype(
      0.001f * (imuDataDelayed.time_ms - lastOriginHgtTime_ms), 0.0, 1.0);
  if (activeHgtSource == AP_NavEKF_Source::SourceZ::BARO) {
    ...
    ekfOriginHgtVar +=
        sq(maxTerrGrad * stateStruct.velocity.xy().length() * deltaTime);
  } else {
  ...
  // calculate the observation variance assuming EKF error relative to datum is
  // independent of GPS observation error when not using GPS as height source
  ...
  // correct the EKF origin and variance estimate if the innovation is less than
  // 5-sigma
...
void NavEKF3_core::readAirSpdData() {
  ...
  // if airspeed reading is valid and is set by the user to be used and has been
  // updated then we take a new reading, convert from EAS to TAS and set the
  // flag letting other functions know a new measurement is available

  if (useAirspeed()) {
    const auto *airspeed = dal.airspeed();
    if (airspeed && (airspeed->last_update_ms(selected_airspeed) -
                     timeTasReceived_ms) > frontend->sensorIntervalMin_ms) {
      ...
      if (tasDataNew.allowFusion) {
        ...
        // Correct for the average intersampling delay due to the filter update
        // rate
    ...
    // Check the buffer for measurements that have been overtaken by the fusion
    // time horizon and need to be fused
    tasDataToFuse = storedTAS.recall(tasDataDelayed, imuDataDelayed.time_ms);
  } else {
    if (is_positive(defaultAirSpeed)) {
      // this is the preferred method with the autopilot providing a model based
      // airspeed estimate
      if (imuDataDelayed.time_ms - prevTasStep_ms > 200) {
        tasDataDelayed.tas = defaultAirSpeed * EAS2TAS;
        tasDataDelayed.tasVariance =
            MAX(defaultAirSpeedVariance, sq(MAX(frontend->_easNoise, 0.5f)));
        tasDataToFuse = true;
      ...
      if (lastAspdEstIsValid && imuDataDelayed.time_ms - prevTasStep_ms > 200) {
        ...
        tasDataDelayed.tasVariance = sq(
            MAX(MAX(frontend->_easNoise, 0.5f), 0.5f * lastAirspeedEstimate));
        tasDataToFuse = true;
        tasDataDelayed.allowFusion = true;
        tasDataDelayed.time_ms = imuDataDelayed.time_ms;
      } else {
...
void NavEKF3_core::readRngBcnData() {
  ...
  static_assert(
      ARRAY_SIZE(rngBcn.lastTime_ms) >= AP_BEACON_MAX_BEACONS,
  ...
  // search through all the beacons for new data and if we find it stop
  // searching and push the data into the observation buffer
  ...
  while (!newDataPushed && (numRngBcnsChecked < rngBcn.N)) {
    ...
    if (beacon->beacon_healthy(index) &&
        beacon->beacon_last_update_ms(index) != rngBcn.lastTime_ms[index]) {
      rng_bcn_elements rngBcnDataNew = {};

      // set the timestamp, correcting for measurement delay and average
      // intersampling delay due to the filter update rate
      rngBcn.lastTime_ms[index] = beacon->beacon_last_update_ms(index);
      rngBcnDataNew.time_ms = rngBcn.lastTime_ms[index] -
                              frontend->_rngBcnDelay_ms -
                              localFilterTimeStep_ms / 2;

      // set the range noise
      // TODO the range library should provide the noise/accuracy estimate for
      // each beacon
      ...
      // if(index == 2)
      // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "beacon new push id:%d r: %0.3f,
      // re:%0.3f ",index, rngBcnDataNew.rng, rngBcnDataNew.rngErr);
    }
  }
  ...
  if ((imuSampleTime_ms - rngBcn.last3DmeasTime_ms < 250) &&
      (rngBcn.vehiclePosErr < 1.0f) && rngBcn.alignmentCompleted) {
    // check for consistency between the position reported by the beacon and the
    // position from the 3-State alignment filter
    const ftype posDiffSq = sq(rngBcn.receiverPos.x - rngBcn.vehiclePosNED.x) +
                            sq(rngBcn.receiverPos.y - rngBcn.vehiclePosNED.y);
    const ftype posDiffVar = sq(rngBcn.vehiclePosErr) +
    ...
    if (posDiffSq < 9.0f * posDiffVar) {
      ...
      if (!validOrigin && (PV_AidingMode != AID_ABSOLUTE)) {
        ...
        if (beacon->get_origin(origin_loc)) {
          ...
          //  set the NE earth magnetic field states using the published
          //  declination and set the corresponding variances and covariances
          alignMagStateDeclination();
  ...
  // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "___BCGood %d imu:%ld BcnLast3D:%ld",
  // rngBcnGoodToAlign, imuSampleTime_ms, rngBcnLast3DmeasTime_ms);
  //  Check the buffer for measurements that have been overtaken by the fusion
  //  time horizon and need to be fused
  rngBcn.dataToFuse =
      rngBcn.storedRange.recall(rngBcn.dataDelayed, imuDataDelayed.time_ms);
  // if(rngBcnDataDelayed.beacon_ID == 2)
  //    GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Delayed id %d x:%.3f y:%.3f",
  //    rngBcnDataDelayed.beacon_ID, rngBcnDataDelayed.beacon_posNED.x,
  //    rngBcnDataDelayed.beacon_posNED.y);
  if (!rngBcn.dataToFuse && beacon->count() != 0) {
    ...
    if (imuSampleTime_ms - last_unfuse_warn_ms > 2000) {
      ...
      GCS_SEND_TEXT(MAV_SEVERITY_INFO, "rngBcnDataNOTToFuse count %d",
                    bcnUnFuseCounter);
    }
  }

  // Correct the range beacon earth frame origin for estimated offset relative
  // to the EKF earth frame origin
  if (rngBcn.dataToFuse) {
    // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "Delayed id %d time %ld x:%.3f y:%.3f",
    // rngBcnDataDelayed.beacon_ID, rngBcnDataDelayed.time_ms,
    // rngBcnDataDelayed.beacon_posNED.x, rngBcnDataDelayed.beacon_posNED.y);
...
void NavEKF3_core::writeEulerYawAngle(float yawAngle, float yawAngleErr,
                                      uint32_t timeStamp_ms, uint8_t type) {
  // limit update rate to maximum allowed by sensor buffers and fusion process
  // don't try to write to buffer until the filter has been initialised
  if (((timeStamp_ms - yawMeasTime_ms) < frontend->sensorIntervalMin_ms) ||
      !statesInitialised) {
    return;
  }
...
// Writes the default equivalent airspeed and 1-sigma uncertainty in m/s to be
// used in forward flight if a measured airspeed is required and not available.
...
void NavEKF3_core::writeExtNavData(const Vector3f &pos, const Quaternion &quat,
                                   float posErr, float angErr,
                                   uint32_t timeStamp_ms, uint16_t delay_ms,
  ...
  if (!quat.is_nan() && !isnan(angErr)) {
    ...
    extNavYawAngDataNew.yawAngErr =
        MAX(angErr, radians(5.0f)); // ensure yaw accuracy is no better than 5
                                    // degrees (some callers may send zero)
    extNavYawAngDataNew.order =
        rotationOrder::TAIT_BRYAN_321; // Euler rotation order is 321 (ZYX)
...
void NavEKF3_core::writeExtNavVelData(const Vector3f &vel, float err,
                                      uint32_t timeStamp_ms,
                                      uint16_t delay_ms) {
#if EK3_FEATURE_EXTERNAL_NAV
...
void NavEKF3_core::update_mag_selection(void) {
  ...
  if (frontend->_affinity & EKF_AFFINITY_MAG) {
    if (core_index < compass.get_count() && compass.healthy(core_index) &&
...
void NavEKF3_core::update_baro_selection(void) {
  ...
  if (frontend->_affinity & EKF_AFFINITY_BARO) {
    if (core_index < baro.num_instances() && baro.healthy(core_index)) {
...
void NavEKF3_core::update_airspeed_selection(void) {
  ...
  if (frontend->_affinity & EKF_AFFINITY_ARSP) {
    if (core_index < arsp->get_num_sensors() && arsp->healthy(core_index) &&
        arsp->use(core_index)) {
...
void NavEKF3_core::learnInactiveBiases(void) {
  ...
  for (uint8_t i = 0; i < INS_MAX_INSTANCES; i++) {
    ...
    if (gyro_index_active == i) {
      ...
      Vector3F filtered_gyro_active =
          ins.get_gyro(gyro_index_active).toftype() -
          (stateStruct.gyro_bias / dtEkfAvg);
      Vector3F filtered_gyro_inactive =
          ins.get_gyro(i).toftype() - (inactiveBias[i].gyro_bias / dtEkfAvg);
      ...
      // slowly bring the inactive gyro in line with the active gyro. This
      // corrects a 5 deg/sec gyro bias error in around 1 minute
  ...
  for (uint8_t i = 0; i < INS_MAX_INSTANCES; i++) {
    ...
    if (accel_index_active == i) {
      ...
      // to move the inactive bias towards the right value
      Vector3F filtered_accel_active =
          ins.get_accel(accel_index_active).toftype() -
          (stateStruct.accel_bias / dtEkfAvg);
      Vector3F filtered_accel_inactive =
          ins.get_accel(i).toftype() - (inactiveBias[i].accel_bias / dtEkfAvg);
...
void NavEKF3_core::updateMovementCheck(void) {
  ...
  const auto &ins = dal.ins();
  Vector3F gyro = ins.get_gyro(gyro_index_active).toftype() -
                  stateStruct.gyro_bias * dtEkfAvgInv;
  Vector3F accel = ins.get_accel(accel_index_active).toftype() -
                   stateStruct.accel_bias * dtEkfAvgInv;
  ...
  } else if (gyro_length_ratio <
                 frontend->_ognmTestScaleFactor * hysteresis_ratio &&
             fabsF(accel_length_ratio) <
                 frontend->_ognmTestScaleFactor * hysteresis_ratio &&
             gyro_diff_ratio <
                 frontend->_ognmTestScaleFactor * hysteresis_ratio &&
             accel_diff_ratio <
...
void NavEKF3_core::SampleDragData(const imu_elements &imu) {
  ...
  // down-sample the drag specific force data by accumulating and calculating
  // the mean when sufficient samples have been collected
...
void NavEKF3_core::getEarthFieldTable(const Location &loc) {
  ...
  table_declination = radians(
      AP_Declination::get_declination(loc.lat * 1.0e-7, loc.lng * 1.0e-7));
