...
void NavEKF3_core::controlFilterModes() {
  ...
  // Determine if learning of wind and magnetic field will be enabled and set
  // corresponding indexing limits to avoid unnecessary operations
...
NavEKF3_core::MagCal NavEKF3_core::effective_magCal(void) const {
  ...
  // handle deprecated MagCal::EXTERNAL_YAW and MagCal::EXTERNAL_YAW_FALLBACK
  // values
...
void NavEKF3_core::setWindMagStateLearningMode() {
  const bool canEstimateWind =
      ((finalInflightYawInit && dragFusionEnabled) || assume_zero_sideslip()) &&
      !onGround && PV_AidingMode != AID_NONE;
  if (!inhibitWindStates && !canEstimateWind) {
    inhibitWindStates = true;
    lastAspdEstIsValid = false;
    updateStateIndexLim();
  } else if (inhibitWindStates && canEstimateWind &&
             (sq(stateStruct.velocity.x) + sq(stateStruct.velocity.y) >
                  sq(5.0f) ||
              dragFusionEnabled)) {
    ...
    if (yawAlignComplete && assume_zero_sideslip()) {
      // if we have a valid heading, set the wind states to the reciprocal of
      // the vehicle heading which assumes the vehicle has launched into the
      // wind use airspeed if if recent data available
      Vector3F tempEuler;
      stateStruct.quat.to_euler(tempEuler.x, tempEuler.y, tempEuler.z);
      ftype trueAirspeedVariance;
      const bool haveAirspeedMeasurement =
          (tasDataDelayed.allowFusion &&
           (imuDataDelayed.time_ms - tasDataDelayed.time_ms < 500) &&
           useAirspeed());
      if (haveAirspeedMeasurement) {
        trueAirspeedVariance =
            constrain_ftype(tasDataDelayed.tasVariance, WIND_VEL_VARIANCE_MIN,
                            WIND_VEL_VARIANCE_MAX);
        const ftype windSpeed =
            sqrtF(sq(stateStruct.velocity.x) + sq(stateStruct.velocity.y)) -
            tasDataDelayed.tas;
        stateStruct.wind_vel.x = windSpeed * cosF(tempEuler.z);
        stateStruct.wind_vel.y = windSpeed * sinF(tempEuler.z);
      } else {
        trueAirspeedVariance =
            sq(WIND_VEL_VARIANCE_MAX); // use 2-sigma for faster initial
                                       // convergence
      }
      ...
      // set the variances using a typical max wind speed for small UAV
      // operation
      zeroCols(P, 22, 23);
      zeroRows(P, 22, 23);
  ...
  // Determine if learning of magnetic field states has been requested by the
  // user
  bool magCalRequested =
      ((effectiveMagCal == MagCal::WHEN_FLYING) && inFlight) || // when flying
      ((effectiveMagCal == MagCal::WHEN_MANOEUVRING) &&
       manoeuvring) || // when manoeuvring
      ((effectiveMagCal == MagCal::AFTER_FIRST_CLIMB) && finalInflightYawInit &&
       finalInflightMagInit) || // when initial in-air yaw and mag field reset
                                // is complete
      (effectiveMagCal == MagCal::ALWAYS); // all the time

  // Deny mag calibration request if we aren't using the compass, it has been
  // inhibited by the user, we do not have an absolute position reference or are
  // on the ground (unless explicitly requested by the user)
  ...
  if (!inhibitMagStates && setMagInhibit) {
    ...
    updateStateIndexLim();
    if (magFieldLearned) {
      // if we have already learned the field states, then retain the learned
      // variances
  ...
  // If on ground we clear the flag indicating that the magnetic field in-flight
  // initialisation has been completed because we want it re-done for each
  // takeoff
  if (onGround) {
...
// Adjust the indexing limits used to address the covariance, states and other
// EKF arrays to avoid unnecessary operations if we are not using those states
...
void NavEKF3_core::setYawSource() {
  AP_NavEKF_Source::SourceYaw yaw_source =
      frontend->sources.getYawSource(core_index);
...
void NavEKF3_core::setAidingMode() {
  ...
  // Handle the special case where we are on ground and disarmed without a yaw
  // measurement and navigating. This can occur if not using a magnetometer and
  // yaw was aligned using GPS during the previous flight.
  if (yaw_source_last == AP_NavEKF_Source::SourceYaw::NONE && !motorsArmed &&
      onGround && PV_AidingMode != AID_NONE) {
  ...
  case AID_NONE: {
    // Don't allow filter to start position or velocity aiding until the tilt
    // and yaw alignment is complete and IMU gyro bias estimates have stabilised
    // If GPS usage has been prohiited then we use flow aiding provided optical
    // flow data is present GPS aiding is the preferred option unless excluded
    // by the user
  ...
  case AID_RELATIVE: {
    // Check if the fusion has timed out (flow measurements have been rejected
    // for too long)
    bool flowFusionTimeout = ((imuSampleTime_ms - prevFlowFuseTime_ms) > 5000);
    // Check if the fusion has timed out (body odometry measurements have been
    // rejected for too long)
    bool bodyOdmFusionTimeout =
        ((imuSampleTime_ms - prevBodyVelFuseTime_ms) > 5000);
    // Enable switch to absolute position mode if GPS or range beacon data is
    // available If GPS or range beacons data is not available and flow fusion
    // has timed out, then fall-back to no-aiding
    if (readyToUseGPS() || readyToUseRangeBeacon() || readyToUseExtNav()) {
      PV_AidingMode = AID_ABSOLUTE;
  ...
  case AID_ABSOLUTE: {
    ...
    uint16_t minTestTime_ms = MIN(
        frontend->tiltDriftTimeMax_ms,
        MIN(frontend->posRetryTimeNoVel_ms, frontend->posRetryTimeUseVel_ms));

    // Check if optical flow data is being used
    bool optFlowUsed =
        (imuSampleTime_ms - prevFlowFuseTime_ms <= minTestTime_ms);

    // Check if body odometry data is being used
    bool bodyOdmUsed =
        (imuSampleTime_ms - prevBodyVelFuseTime_ms <= minTestTime_ms);
    ...
    const bool rngBcnUsed =
        (imuSampleTime_ms - rngBcn.lastPassTime_ms <= minTestTime_ms);
#else
    ...
    bool attAiding = posUsed || gpsVelUsed || optFlowUsed || airSpdUsed ||
                     dragUsed || rngBcnUsed || bodyOdmUsed;

    // Check if velocity drift has been constrained by a measurement source
    // Currently these are all the same source as will stabilise attitude
    // because we do not currently have a sensor that only observes attitude
    velAiding = posUsed || gpsVelUsed || optFlowUsed || airSpdUsed ||
                dragUsed || rngBcnUsed || bodyOdmUsed;

    // Store the last valid airspeed estimate
    windStateIsObservable =
        !inhibitWindStates &&
        (posUsed || gpsVelUsed || optFlowUsed || rngBcnUsed || bodyOdmUsed);
    if (windStateIsObservable) {
      lastAirspeedEstimate =
          (stateStruct.velocity -
           Vector3F(stateStruct.wind_vel.x, stateStruct.wind_vel.y, 0.0F))
              .length();
    ...
    if (!attAiding) {
      attAidLossCritical = (imuSampleTime_ms - prevFlowFuseTime_ms >
                            frontend->tiltDriftTimeMax_ms) &&
                           (imuSampleTime_ms - lastTasPassTime_ms >
                            frontend->tiltDriftTimeMax_ms) &&
#if EK3_FEATURE_BEACON_FUSION
                           (imuSampleTime_ms - rngBcn.lastPassTime_ms >
                            frontend->tiltDriftTimeMax_ms) &&
#endif
                           (imuSampleTime_ms - lastGpsPosPassTime_ms >
                            frontend->tiltDriftTimeMax_ms) &&
                           (imuSampleTime_ms - lastVelPassTime_ms >
                            frontend->tiltDriftTimeMax_ms);
    }
    ...
    if (attAidLossCritical) {
      // if the loss of attitude data is critical, then put the filter into a
      ...
      // if the loss of position is critical, declare all sources of position
      // aiding as being timed out
      posTimeout = true;
  ...
  // check to see if we are starting or stopping aiding and set states and modes
  // as required
  if (PV_AidingMode != PV_AidingModePrev) {
    // set various usage modes based on the condition when we start aiding.
    // These are then held until aiding is stopped.
    switch (PV_AidingMode) {
    case AID_NONE:
      // We have ceased aiding
      GCS_SEND_TEXT(MAV_SEVERITY_WARNING, "EKF3 IMU%u stopped aiding",
                    (unsigned)imu_index);
      // When not aiding, estimate orientation & height fusing synthetic
      // constant position and zero velocity measurement to constrain tilt
      // errors
      posTimeout = true;
      velTimeout = true;
      ...
      velTestRatio = 0.0f;
      posTestRatio = 0.0f;
      // store the current position to be used to keep reporting the last known
      // position
      lastKnownPositionNE.x = stateStruct.position.x;
      lastKnownPositionNE.y = stateStruct.position.y;
      // initialise filtered altitude used to provide a takeoff reference to
      // current baro on disarm this reduces the time required for the baro
      // noise filter to settle before the filtered baro data can be used
      meaHgtAtTakeOff = baroDataDelayed.hgt;
      // reset the vertical position state to faster recover from baro errors
      // experienced during touchdown
    ...
    case AID_RELATIVE:
      // We are doing relative position navigation where velocity errors are
      // constrained, but position drift will occur
      GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u started relative aiding",
                    (unsigned)imu_index);
#if EK3_FEATURE_OPTFLOW_FUSION
    ...
    case AID_ABSOLUTE:
      if (readyToUseGPS()) {
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u is using GPS",
                      (unsigned)imu_index);
#if EK3_FEATURE_BEACON_FUSION
        ...
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                      ...
                      (unsigned)imu_index);
        GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                      "EKF3 IMU%u initial pos NED = %3.1f,%3.1f,%3.1f (m)",
                      (unsigned)imu_index, (double)extNavDataDelayed.pos.x,
                      (double)extNavDataDelayed.pos.y,
                      (double)extNavDataDelayed.pos.z);
        if (useExtNavVel) {
          velResetSource = resetDataSource::EXTNAV;
          GCS_SEND_TEXT(MAV_SEVERITY_INFO,
                        "EKF3 IMU%u initial vel NED = %3.1f,%3.1f,%3.1f (m/s)",
                        (unsigned)imu_index, (double)extNavVelDelayed.vel.x,
                        (double)extNavVelDelayed.vel.y,
                        (double)extNavVelDelayed.vel.z);
        }
        // handle height reset as special case
        hgtMea = -extNavDataDelayed.pos.z;
        posDownObsNoise =
            sq(constrain_ftype(extNavDataDelayed.posErr, 0.1f, 10.0f));
        ResetHeight();
#endif // EK3_FEATURE_EXTERNAL_NAV
      }

      // clear timeout flags as a precaution to avoid triggering any additional
      // transitions
      posTimeout = false;
      velTimeout = false;

      // reset the last fusion accepted times to prevent unwanted activation of
      // timeout logic
...
void NavEKF3_core::checkAttitudeAlignmentStatus() {
  ...
  // Once the tilt variances have reduced, re-set the yaw and magnetic field
  // states and declare the tilt alignment complete
  if (!tiltAlignComplete) {
    if (tiltErrorVariance < sq(radians(5.0))) {
      tiltAlignComplete = true;
      GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u tilt alignment complete",
                    (unsigned)imu_index);
    }
  }
...
bool NavEKF3_core::useRngFinder(void) const {
  // TO-DO add code to set this based in setting of optical flow use parameter
...
bool NavEKF3_core::readyToUseOptFlow(void) const {
  ...
  // We need stable roll/pitch angles and gyro bias estimates but do not need
  // the yaw angle aligned to use optical flow
  return (imuSampleTime_ms - flowMeaTime_ms < 200) && tiltAlignComplete &&
         delAngBiasLearned;
}
#endif // EK3_FEATURE_OPTFLOW_FUSION

// return true if the filter is ready to start using body frame odometry
// measurements
bool NavEKF3_core::readyToUseBodyOdm(void) const {
#if EK3_FEATURE_BODY_ODOM
  if (!frontend->sources.useVelXYSource(AP_NavEKF_Source::SourceXY::EXTNAV,
                                        core_index) &&
      !frontend->sources.useVelXYSource(
          AP_NavEKF_Source::SourceXY::WHEEL_ENCODER, core_index)) {
    // exit immediately if sources not configured to fuse external nav or wheel
    // encoders
    return false;
  }

  // Check for fresh visual odometry data that meets the accuracy required for
  // alignment
  bool visoDataGood = (imuSampleTime_ms - bodyOdmMeasTime_ms < 200) &&
                      (bodyOdmDataNew.velErr < 1.0f);

  // Check for fresh wheel encoder data
  bool wencDataGood =
      (imuDataDelayed.time_ms - wheelOdmDataDelayed.time_ms < 200);

  // We require stable roll/pitch angles and gyro bias estimates but do not need
  // the yaw angle aligned to use odometry measurements because they are in a
  // body frame of reference
  return (visoDataGood || wencDataGood) && tiltAlignComplete &&
         delAngBiasLearned;
#else
  return false;
...
bool NavEKF3_core::readyToUseGPS(void) const {
  ...
  return validOrigin && tiltAlignComplete && yawAlignComplete &&
         (delAngBiasLearned || assume_zero_sideslip()) && gpsGoodToAlign &&
         gpsDataToFuse;
}
...
bool NavEKF3_core::using_noncompass_for_yaw(void) const {
  ...
  if (yaw_source_last == AP_NavEKF_Source::SourceYaw::EXTNAV) {
    return ((imuSampleTime_ms - last_extnav_yaw_fusion_ms < 5000) ||
            (imuSampleTime_ms - lastSynthYawTime_ms < 5000));
  }
#endif
  if (yaw_source_last == AP_NavEKF_Source::SourceYaw::GPS ||
      yaw_source_last == AP_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK ||
      yaw_source_last == AP_NavEKF_Source::SourceYaw::GSF || !use_compass()) {
    return imuSampleTime_ms - last_gps_yaw_ms < 5000 ||
           imuSampleTime_ms - lastSynthYawTime_ms < 5000;
  }
  return false;
}
...
bool NavEKF3_core::assume_zero_sideslip(void) const {
  ...
  // traction is lost
  return dal.get_fly_forward() &&
         dal.get_vehicle_class() != AP_DAL::VehicleClass::GROUND;
}

// sets the local NED origin using a LLH location (latitude, longitude, height)
// returns false if the origin is already set
bool NavEKF3_core::setOriginLLH(const Location &loc) { return setOrigin(loc); }
...
bool NavEKF3_core::setOrigin(const Location &loc) {
  ...
  GCS_SEND_TEXT(MAV_SEVERITY_INFO, "EKF3 IMU%u origin set",
                (unsigned)imu_index);
...
// set the class variable true if the delta angle bias variances are
// sufficiently small
void NavEKF3_core::checkGyroCalStatus(void) {
  // check delta angle bias variances
  const ftype delAngBiasVarMax = sq(radians(0.15 * dtEkfAvg));
  if (!use_compass() && (yaw_source_last != AP_NavEKF_Source::SourceYaw::GPS) &&
      (yaw_source_last != AP_NavEKF_Source::SourceYaw::GPS_COMPASS_FALLBACK) &&
      (yaw_source_last != AP_NavEKF_Source::SourceYaw::EXTNAV)) {
    // rotate the variances into earth frame and evaluate horizontal terms only
    // as yaw component is poorly observable without a yaw reference which can
...
void NavEKF3_core::updateFilterStatus(void) {
  ...
  status.value = 0;
  bool doingBodyVelNav = (PV_AidingMode != AID_NONE) &&
                         (imuSampleTime_ms - prevBodyVelFuseTime_ms < 5000);
  bool doingFlowNav = (PV_AidingMode != AID_NONE) && flowDataValid;
  bool doingWindRelNav =
      (!tasTimeout && assume_zero_sideslip()) || !dragTimeout;
  bool doingNormalGpsNav = !posTimeout && (PV_AidingMode == AID_ABSOLUTE);
  bool someVertRefData =
      (!velTimeout && (useGpsVertVel || useExtNavVel)) || !hgtTimeout;
  bool someHorizRefData =
      !(velTimeout && posTimeout && tasTimeout && dragTimeout) ||
      doingFlowNav || doingBodyVelNav;
  bool filterHealthy =
      healthy() && tiltAlignComplete &&
      (yawAlignComplete || (!use_compass() && (PV_AidingMode != AID_ABSOLUTE)));

  // If GPS height usage is specified, height is considered to be inaccurate
  // until the GPS passes all checks
  bool hgtNotAccurate = (frontend->sources.getPosZSource(core_index) ==
                         AP_NavEKF_Source::SourceZ::GPS) &&
                        !validOrigin;

  // set individual flags
  status.flags.attitude =
      !stateStruct.quat.is_nan() &&
      filterHealthy; // attitude valid (we need a better check)
  status.flags.horiz_vel =
      someHorizRefData && filterHealthy; // horizontal velocity estimate valid
  status.flags.vert_vel =
      someVertRefData && filterHealthy; // vertical velocity estimate valid
...
#endif
  status.flags.horiz_pos_rel =
      ((doingFlowNav && optflow_gnd_offset) || doingWindRelNav ||
       doingNormalGpsNav || doingBodyVelNav) &&
      filterHealthy; // relative horizontal position estimate valid

  status.flags.horiz_pos_abs =
      doingNormalGpsNav &&
      filterHealthy; // absolute horizontal position estimate valid
  status.flags.vert_pos = !hgtTimeout && filterHealthy &&
                          !hgtNotAccurate; // vertical position estimate valid
  status.flags.terrain_alt =
      gndOffsetValid && filterHealthy; // terrain height estimate valid
  status.flags.const_pos_mode =
      (PV_AidingMode == AID_NONE) && filterHealthy; // constant position mode
  status.flags.pred_horiz_pos_rel =
      status.flags.horiz_pos_rel; // EKF3 enters the required mode before flight
  status.flags.pred_horiz_pos_abs =
      status.flags.horiz_pos_abs; // EKF3 enters the required mode before flight
  status.flags.takeoff_detected =
      takeOffDetected; // takeoff for optical flow navigation has been detected
  status.flags.takeoff =
      dal.get_takeoff_expected(); // The EKF has been told to expect takeoff is
                                  // in a ground effect mitigation mode and has
                                  // started the EKF-GSF yaw estimator
  status.flags.touchdown =
      dal.get_touchdown_expected(); // The EKF has been told to detect touchdown
                                    // and is in a ground effect mitigation mode
  status.flags.using_gps =
      ((imuSampleTime_ms - lastGpsPosPassTime_ms) < 4000) &&
      (PV_AidingMode == AID_ABSOLUTE);
  status.flags.gps_glitching =
      !gpsAccuracyGood && (PV_AidingMode == AID_ABSOLUTE) &&
      (frontend->sources.getPosXYSource(core_index) ==
       AP_NavEKF_Source::SourceXY::GPS); // GPS glitching is affecting
                                         // navigation accuracy
  status.flags.gps_quality_good = gpsGoodToAlign;
  // for reporting purposes we report rejecting airspeed after 3s of not fusing
  // when we want to fuse the data
  status.flags.rejecting_airspeed =
  ...
  status.flags.dead_reckoning = (PV_AidingMode != AID_NONE) &&
                                doingWindRelNav &&
                                !((doingFlowNav && gndOffsetValid) ||
                                  doingNormalGpsNav || doingBodyVelNav);
...
void NavEKF3_core::runYawEstimatorPrediction() {
  ...
  yawEstimator->update(imuDataDelayed.delAng, imuDataDelayed.delVel,
                       imuDataDelayed.delAngDT, imuDataDelayed.delVelDT,
                       EKFGSF_run_filterbank, trueAirspeed);
}
...
void NavEKF3_core::runYawEstimatorCorrection() {
  ...
  if (EKFGSF_run_filterbank) {
    if (gpsDataToFuse) {
      ...
      ftype gpsVelAcc =
          fmaxF(gpsSpdAccuracy, ftype(frontend->_gpsHorizVelNoise));
      yawEstimator->fuseVelData(gpsVelNE, gpsVelAcc);

      // after velocity data has been fused the yaw variance estimate will have
      // been refreshed and is used maintain a history of validity
    ...
    if (EKFGSF_yaw_reset_request_ms > 0 &&
        imuSampleTime_ms - EKFGSF_yaw_reset_request_ms <
