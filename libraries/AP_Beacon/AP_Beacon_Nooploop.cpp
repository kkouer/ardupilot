...
#define NOOPLOOP_NODE_FRAME2_FRAMELEN_MAX                                      \
  4096 // frames should be less than 4k bytes
#define NOOPLOOP_NODE_FRAME2_SYSTIME                                           \
  6 // start of 4 bytes holding system time in ms
#define NOOPLOOP_NODE_FRAME2_PRECISION_X                                       \
  10 // start of 1 byte holding precision in m*100 in x axis
#define NOOPLOOP_NODE_FRAME2_PRECISION_Y                                       \
  11 // start of 1 byte holding precision in m*100 in y axis
#define NOOPLOOP_NODE_FRAME2_PRECISION_Z                                       \
  12 // start of 1 byte holding precision in m*100 in y axis
#define NOOPLOOP_NODE_FRAME2_POSX                                              \
  13 // start of 3 bytes holding x position in m*1000
#define NOOPLOOP_NODE_FRAME2_POSY                                              \
  16 // start of 3 bytes holding y position in m*1000
#define NOOPLOOP_NODE_FRAME2_POSZ                                              \
  19 // start of 3 bytes holding z position in m*1000
  ...
  while (nbytes-- > 0) {
    ...
    if (b >= 0) {
      ...
      if (type == MsgType::NODE_FRAME2) {
        // GCS_SEND_TEXT(MAV_SEVERITY_INFO, "count %d beacon",count); kkouer
        // mod, just pares the frame directly if (_anchor_pos_avail &&
        // isParsedSettingFrame) {
        parse_node_frame2();
...
void AP_Beacon_Nooploop::request_setting() {
  ...
  //      uart->write((uint8_t)0); //manual states filled with any char, but in
  //      fact only 0 works
  //  }
  ...
  uint8_t bytes[] = {
      0x54, 0x00, 0x01, 00, 00, 00,   00,   00,   00,   00,   00,  00, 00,
      0xff, 00,   0xff, 00, 00, 0xff, 0xff, 00,   0xff, 0xff, 00,  00, 00,
      00,   00,   00,   00, 00, 0xff, 0xff, 0xff, 0xff, 0xff, 00,  00, 00,
      ...
      00,   00,   00,   00, 00, 00,   00,   00,   00,   00,   00,  00, 00,
      00,   00,   00,   00, 00, 00,   00,   00,   00,   00,   00,  00, 00,
      00,   00,   00,   00, 00, 00,   00,   00,   00,   00,   0x4a};
...
void AP_Beacon_Nooploop::parse_node_frame2() {
  ...
  // EKF's estimate goes very bad if the error value sent into the EKF is
  // unrealistically low. ensure it's never less than a reasonable value
  const float pos_err = MAX(0.1f, sqrtf(sq(precision_x) + sq(precision_y)));

  // x,y,z position in m*1000 in ENU frame
  const int32_t pos_x = ((int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSX + 2] << 24 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSX + 1] << 16 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSX] << 8) >>
                        8;
  const int32_t pos_y = ((int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSY + 2] << 24 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSY + 1] << 16 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSY] << 8) >>
                        8;
  const int32_t pos_z = ((int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSZ + 2] << 24 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSZ + 1] << 16 |
                         (int32_t)_msgbuf[NOOPLOOP_NODE_FRAME2_POSZ] << 8) >>
                        8;
  ...
  bool should_print_rssi = (_last_update_ms - last_rssi_print_ms) > 1000;
  ...
  for (uint8_t i = 0; i < valid_nodes; i++) {
    ...
    uint8_t id = _msgbuf[offset + 1]; // nooploop id starts from 0, increments
                                      // clockwise, 0 -> 1 define Y axis.
    const int32_t dist = ((int32_t)_msgbuf[offset + 2 + 2] << 24 |
                          (int32_t)_msgbuf[offset + 2 + 1] << 16 |
...
void AP_Beacon_Nooploop::parse_setting_frame0() {
  ...
  for (uint8_t i = 0; i < 4; i++) {
    ...
    // x,y,z position in m*1000 in ENU frame
    const int32_t pos_x =
        ((int32_t)_msgbuf[offset + 2] << 24 |
         (int32_t)_msgbuf[offset + 1] << 16 | (int32_t)_msgbuf[offset] << 8) >>
        8;
